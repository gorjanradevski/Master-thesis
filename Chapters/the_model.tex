...
\begin{itemize}
    ...
    \item The per pixel mean is subtracted from the image according to \citet{krizhevsky2012imagenet}.
\end{itemize}
...
\end{itemize}
By lowercasing and removing the punctuation, it is assured that the words are consistent throughout the dataset, and a word as a starting word or at the end of the sentence is treated as the same word.  
...
For the language model later used to obtain \textit{ELMo}, 2 layers $(L = 2)$ of forward and backward \textit{LSTMs} are stacked on top of each other, followed by a softmax that returns return the probability for the next word in the sequence.\endgraf
...
Ideally, the end goal is each attention hop to focus on attending to the smallest possible number of distinct entities present in the image or the sentence. Therefore, the penalization term to enforse diversity between the attention weights is computed as:
...
In the most extreme case, when both attention hops attend on the same vector and the attention weights $a_i$ and $a_j$ are equivalent, the dot product is $1$. However, if we consider the opposite case where both attention hops attend on a unique vector, the dot product is $0$, which is the desired outcome.\endgraf
