...
Figure \ref{fig:image_attn_1} depicts the visual attention across 11 distinct attention hops. As we can see, the raw image consists of two different coloured dogs that are playing with a frisbee. Because of the fine-grained nature of the image, we can observe that the first three visual attention hops (the second, third and fourth image), attend to the general outline of the image, or in other words, the attention weights are spread on all 3 entities. The fourth, fifth, sixth and seventh attention hop (the 4 images in the second row) attend mainly to the black dog and the frisbee. Lastly, the final four attention hops (the 4 images in the third row) attend to the brown dog.
...
\begin{figure}
  ...
  \caption[Visual attention of a model trained with a penalization term]{Visual attention when the model is trained together \textbf{with} minimizing a term to ensure diversity between the attention hops.}
